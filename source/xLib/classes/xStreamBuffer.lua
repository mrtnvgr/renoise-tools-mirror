...
The class maintains three buffers - one for input, output and scheduled content. 
Schedule content to place it ahead of the stream position. 

TODO scheduled content is merged into the output - so, for a realistic example, scheduling an LFO reset at the beginning of a pattern, you would want to define a 'sparse' xline, containing just the effect column with a reset command. Otherwise, other parts of the line would be replaced too. 
...
function xStreamBuffer:__init(xpos)
  ...
  --- int, decide which track to read from (0 = none)
  ...
  --- int, decide which track to write to (0 = none)
...
function xStreamBuffer:schedule_note_column(xnotecol,col_idx,xinc)
  ...
  local xline = self:read_input(xinc,self:_get_songpos(xinc)) 
...
function xStreamBuffer:_create_content(num_lines,xinc)
  ...
  for i = 0, num_lines-1 do
    ...
    -- retrieve content (callback, pattern)
    local xline = self:read_input(xinc,pos)
...
---------------------------------------------------------------------------------------------------
-- Get input
-- 1. obtain from scheduled content buffer
-- 2. obtain via callback function
-- 3. read from pattern 
-- @param xinc (int), the buffer position
-- @param [pos] (SongPos), where to read from song
-- @return xLine, xline descriptor (never nil)

function xStreamBuffer:read_input(xinc,pos)
  TRACE("xStreamBuffer:read_input(xinc,pos)",xinc,pos)
  ...
  --elseif pos then
    ...
    self.input_buffer[xinc] = table.rcopy(xline) 
  end
  if not xline then
    xline = self.empty_xline
  end
